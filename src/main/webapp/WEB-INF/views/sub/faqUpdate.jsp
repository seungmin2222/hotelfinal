...
<!-- 	<div class="sub_wrapper"> -->
		...
		<form action="${pageContext.request.contextPath}/sub/faqUpdatePro" method="post" style="margin-top: 20px">
			<input type="hidden" name="faq_idx" value="${faqBoardDTO.faq_idx}">
			<table>
			<tr style="margin : auto;"><td></td><td><input type="text" name="faq_subject" value="${faqBoardDTO.faq_subject }" placeholder="제목" style="width: 500px; height:40px; margin-bottom: 20px; border: 1px solid #9F8562; ">
			<c:set var="category" value="${faqBoardDTO.faq_category }" />
			<c:choose>
				<c:when test="${category eq '객실' }">
					<select name="faq_category" style="margin-left: 30px; height: 40px;">
			...
			<tr style="margin : auto;"><td></td><td><textarea name="faq_content"  rows="15" cols="120" style="resize: both; border:1px solid #9F8562; ">${faqBoardDTO.faq_content}</textarea></td></tr>
			</table>
